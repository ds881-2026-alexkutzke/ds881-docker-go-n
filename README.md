...
Este repositório contém o código-fonte de uma API REST de Calculadora desenvolvida em Go utilizando o framework Gin. O gerenciamento de pacotes é realizado pelo Go Modules (go.mod), exigindo o download das dependências (go mod download) antes da compilação do binário. A aplicação expõe a porta 8080.
